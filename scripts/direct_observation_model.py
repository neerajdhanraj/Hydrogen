#!/usr/bin/env python3
"""Independent spatial validation of hydrogen-resilience mechanisms.

Electricity network: official PyPSA-Eur OSM v0.6 high-voltage grid aggregated
only at country interfaces. Countries without strict OPSD chronology coverage
remain passive transit nodes; no energy profiles are imputed for them.

Chronology: independent OPSD/ENTSO-E hourly load, wind and solar series for the
12 countries passing a >=95% completeness rule in every year 2015-2019.
Profiles are preprocessed by prepare_external_opsd_profiles.py so annual demand
growth and renewable build-out are removed while chronology is retained.

The model is deliberately a fixed-capacity linear adequacy dispatch benchmark,
not a forecast of the historical 2015-2019 European generation fleet.  2015 is
used only to calibrate one global firm-capacity margin.  2016-2019 are frozen,
held-out external test years.  Hydrogen scale is transferred from the reduced
benchmark using dimensionless fractions of annual electricity and mean load.
...
def load_inputs(profile_path:Path, osm_dir:Path):
    ...
    countries=sorted(buses.country.dropna().unique())
    # Keep only the graph component containing the active countries; all active
    # countries should be in one connected European component via passive nodes.
    G=nx.Graph(); G.add_nodes_from(countries)
    for r in edges.itertuples(index=False):
        if r.total_nominal_mw>0: G.add_edge(r.a,r.b)
    comp=set()
    for cc in active:
        if cc not in G: raise ValueError(f'Active country {cc} absent from OSM graph')
        comp |= nx.node_connected_component(G,cc)
    # Union can include all components if active are split; retain them all.
    countries=sorted(comp)
    edges=edges[edges.a.isin(countries)&edges.b.isin(countries)].copy().reset_index(drop=True)
